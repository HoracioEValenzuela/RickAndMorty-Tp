...
//LOS EXPERIMENTOS NACEN CON LA ESTRATEGIA DE SELECCION AlAzar

class Experimento{
	const materialesNecesarios = #{}
	var estrategiaSeleccionMateriales
	
	
	constructor(estrategiaSeleccion) {
		estrategiaSeleccionMateriales = estrategiaSeleccion
	}
		...
		method addMaterialesNecesarios(condicion){ 
			materialesNecesarios.add(estrategiaSeleccionMateriales.seleccion(condicion))
		}		
...
object construirBateria inherits Experimento(new AlAzar()){
	...
	const condicionDeMetal = { material => material.cantMetal() > 200 }	
		...
		override method agarrarLoQueNecesita(personaje){
			self.addMaterialesNecesarios(condicionDeRadiactivo)
			self.addMaterialesNecesarios(condicionDeMetal)
		}
...
object construirCircuito inherits Experimento(new AlAzar()){
...
object shockElectrico inherits Experimento(new AlAzar()){
	...
	const condicionDeConducirElectricidad = { material => material.conduceElectricidad() }		
		...
		override method agarrarLoQueNecesita(personaje){
			self.addMaterialesNecesarios(condicionDeGenerarElectricidad)
			self.addMaterialesNecesarios(condicionDeConducirElectricidad)
		} 
}
